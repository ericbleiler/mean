...
var db14 = mongojs('songs', ['songs']);
var db15 = mongojs('queue', ['queue']);
...
app.get('/api/employees/queue', function (req, res) {
  db15.queue.find( function (err, queue) {
    res.json(queue);
  });
});
...
app.get('/savedArtists/:artist' , function(req, res){
  db10.savedArtists.findOne({'artist': req.params.artist},function(err, savedArtists){
...
app.get('/music/:artist' , function(req, res){
  db5.music.find({'artist': req.params.artist}, function(err, m){
    res.json(m);  
  }); 
...
app.get('/radio/:artist' , function(req, res){
 db11.radio.find({'artist': req.params.artist}, function(err, radio){
    res.json(radio);  
  });    
});
...
app.get('/search/:artist' , function(req, res){
  db6.artists.find({'artist': req.params.artist}, function(err, a){
    res.json(a);  
  });    
});
app.get('/songs/:artist' , function(req, res){
  db14.songs.find({'artist': req.params.artist}, function(err, t){
    res.json(t);  
  }); 
...
app.post('/queue', function(req, res){
  db15.queue.drop(); 
  db15.queue.insert( req.body, function(err, play){
    console.log(req.body);
    if(err)
      res.send(err);
    res.json(play);
